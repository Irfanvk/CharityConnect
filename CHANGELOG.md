...
|---------|------|--------|----------|
| 2.23 | 2026-06-01 | Patch | Community directory: member ID column hidden from regular-member view — only admin/superadmin see member codes below each name in the directory grid. |
| 2.22 | 2026-06-01 | Minor | **Community page full rewrite:** complete member directory with 3-column responsive grid (`sm:grid-cols-2 lg:grid-cols-3`), search by name or member ID (admin only), scrollable at max 520 px, role-split queries (admin → `GET /members/`, member → `GET /members/community`). Stats row (total/active members, collection, balance). Spending pie chart by category + collection timeline (Today/Week/Month/Year/All-Time). **MemberWatermark component** (`src/components/MemberWatermark.jsx`): fixed translucent repeating diagonal SVG overlay (4.5 % opacity) showing `member_code`/username — invisible in normal use, clearly readable in screenshots for admin traceability; admins and superadmins see no overlay. **Layout.jsx** wrapped in `MemberWatermark` so the watermark covers every page for authenticated members. **MemberDashboard.jsx:** removed confusing "Member list preview is not available for your role yet." message; replaced with "Fellow Members" pill row (up to 10) and "See all members →" link to Community page. |
