...
### 5. **PWA (Progressive Web App) Support** (New - March 6, 2026)
   - Mobile app installation on iOS/Android/Desktop
   - Offline-first caching strategies
   - Install prompt UX and platform guidance
...
## � Progressive Web App (PWA) Support (NEW - v2.0)
...
- **Install as native app** on iOS, Android, Windows, and macOS
- **Work offline** with cached content and API responses
...
**Platform Support:**
- ✅ **Android Chrome**: Native install prompt with app icon on home screen
- ✅ **iOS Safari**: Manual install via "Share → Add to Home Screen" with toast guidance
- ✅ **Desktop Chrome/Edge**: Install as Windows/Mac app
- ✅ **Desktop Firefox**: Limited support (manual add-on)

**User Experience:**
- Install button appears in header when app is installable
- Button hidden after installation
- One-click installation on Android/Desktop via browser install prompt
- Toast guidance for iOS Safari manual installation

#### 2. Service Worker & Offline Caching

**What Gets Cached:**
- **API Calls** (`/api/*`): Network-first strategy with 24h cache
  - Tries network first for fresh data
  - Falls back to cache if offline or timeout (10s)
  - Used for: member data, campaigns, challans, notifications
- **Images**: Stale-while-revalidate with 7d expiration
  - Serves cache immediately, refreshes in background
  - Limited to 30 most recent images
- **Static Assets**: (js, css, html) - Pre-cached on install

**Offline Behavior:**
- User can view cached data even without internet
- Reads, updates, and creates are cached but require sync when online
- Auth status respected (no viewing other users' cached data)
...
**Features:**
- Detects `beforeinstallprompt` event for install-ready state
- Shows button only when installable
- Hidden after installation (detects `display-mode: standalone`)
- Platform detection:
  - Android/Chrome: Triggers native install
  - iOS Safari: Shows toast with manual instructions
  - Already installed: Button hidden
...
PWA is 100% client-side. However, verify these server-side configurations:

#### 1. Static File Serving

**Requirement:** Manifest and icons must be publicly accessible

```
GET /manifest.json         → 200 OK + Content-Type: application/json
GET /icons/icon-192.png    → 200 OK + Content-Type: image/png
GET /icons/icon-512.png    → 200 OK + Content-Type: image/png
```

**FastAPI Setup (Already Configured):**
```python
from fastapi.staticfiles import StaticFiles

app.mount("/icons", StaticFiles(directory="public/icons"), name="icons")
# manifest.json served from public/ root
```

#### 2. CORS Headers (Already Configured)

Service workers require CORS compliance. All API responses should include:
...
**FastAPI middleware already includes this.**

#### 3. Cache-Control Headers (Optional Optimization)

To optimize caching behavior, set appropriate headers:

```python
# For API endpoints that should cache
...
- API calls cache regardless of server headers (24h limit)
- Non-200 responses not cached (`cacheableResponse: { statuses: [0, 200] }`)

#### 4. Service Worker Scope Verification

**SW Scope:** `/` (entire app domain)

Verify backend API is accessible at configured URL:
```
VITE_CHARITY_APP_BASE_URL=http://localhost:8000  (dev)
VITE_CHARITY_APP_BASE_URL=https://api.example.com (prod)
```

#### 5. Session/Auth Considerations

**How PWA Handles Authentication:**

| Scenario | Behavior |
|----------|----------|
| **Online + Valid Token** | Fresh API call, response cached |
| **Offline + Cached Data** | Serves cached data (may be stale) |
| **Online + Expired Token** | 401 response, frontend redirects to login |
| **Offline + No Cache** | Offline page or empty state |

**No backend changes needed** — frontend already handles 401 redirects and token validation.
...
# 3. Click "Install App" button and confirm
...
# 2. Go to http://<your-computer-ip>:5173

# 3. Click "Install App" button

# 4. See toast: "Tap Share → Add to Home Screen"

# 5. Tap Share button (arrow up)
...
# 2. Open app (should load)

# 3. Toggle airplane mode or disconnect WiFi

# 4. Refresh or navigate pages

# 5. Cached data should still load

# 6. Actions requiring network show error
```
...
// Backend origin (production)
urlPattern: backendApiPattern  // Derived from VITE_CHARITY_APP_BASE_URL
...
**Network-First (APIs):**
1. Try network call
2. If timeout or error → use cache
3. Cache indefinitely (24h hard limit)
4. Good for: frequently-updated data that users need fresh

**Stale-While-Revalidate (Images):**
1. Return cache immediately
2. Fetch fresh copy in background
3. Update cache for next load
4. Good for: less critical assets that can be slightly stale
...
| Issue | Solution |
|-------|----------|
| **No install button** | Not using HTTPS (prod) or not valid manifest |
| **Install fails on Android** | Check manifest.json validity with DevTools |
| **iOS manual install unclear** | App requires full-screen tour or setup step |
| **Offline: no data** | Not cached yet (visit online first) or cache cleared |
| **Service worker not updating** | Clear cache or wait (auto-update enabled) |
| **"Add to Home Screen" greyed out** | App must be accessed from home (not iframe) |
