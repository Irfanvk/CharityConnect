...
|------|----------|-------|--------|-------|
| 2026-08-31 | Reports outstanding receivables now displays missing monthly dues | Both | ✅ | The report now receives derived monthly dues for active members when production history contains approved payments but no unpaid challan rows. In the drill-down, response-only derived records are marked `due` with “Missing monthly record,” while actual pending/generated challans retain their existing status. This does not create or modify any challan data. |
| 2026-08-31 | Netlify production redirects and receivables rollout diagnostics corrected | Frontend / Operations | ✅ | Moved HTTP-to-HTTPS redirects ahead of the SPA catch-all so they can take effect. Confirmed `https://poyyathabailgcc.com` is allowed by the live API CORS policy; the `poyyathabailgcc.netlify.app` alias is rejected and must be added to KVM `ALLOWED_ORIGINS` only if it remains a supported access URL. Live KVM still identifies as the older CharityHub backend, so it must pull and restart at the pushed backend revision before the outstanding-receivables production enum fix can return legacy pending/generated values. |
| 2026-08-31 | Reports now surfaces outstanding-receivables API failures instead of showing a misleading zero | Frontend | ✅ | Admin and superadmin Reports now shows the returned API error with a Retry action when `GET /challans/outstanding` fails. The financial summary remains available, but a failed production receivables query is no longer indistinguishable from a genuine ₹0 balance. Production must deploy the backend enum-label compatibility fix (`014b9c4` or later) and allow every active frontend origin through CORS. |
| 2026-08-31 | Superadmin Panel reorganized into task-focused administration options | Frontend | ✅ | The panel now opens with a concise option list for Role Management and Activity & Notification Monitoring. Selecting an option loads only that detailed workspace, and a back action returns to the option list. Existing role controls, activity data, and monitoring behavior are unchanged; their queries now run only when the related option is opened. |
| 2026-08-31 | Notification delivery now uses a background queue when available | Backend | ✅ | Web Push delivery for persisted notifications is queued through Celery so admin broadcasts and request moderation do not wait for external push services. The frontend contract is unchanged: notifications remain immediately visible through the existing realtime/feed paths, while offline push delivery runs in the background. New member requests now notify active admins and superadmins. |
| 2026-08-31 | Superadmin activity and notification-enrollment monitoring added | Both | ✅ | The Superadmin Panel now includes a separate Activity & Notification Monitoring view for members, admins, and superadmins. It shows last authenticated activity in IST, browser notification permission, installed-PWA/browser mode, active push-device count, and global Web Push configuration status. The client reports its current permission state when an authenticated user opens the updated app; `enabled` confirms an active push subscription, while blocked, not requested, needs resync, unsupported, and not reported states remain visible. Subscription endpoints and encryption keys are never shown in the UI. |
